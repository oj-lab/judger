use crate::agent::http::HttpClient;
...
impl JudgeWorker {
    pub fn new(
        ...
        let platform_client = HttpClient::new(base_url);
    ...
    pub async fn run(&self) {
        ...
        loop {
            interval.tick().await;
            match pick_task(&self.platform_client).await {
                Ok(task) => {
                    ...
                    match self.run_judge(task) {
                        Ok(results) => {
                            let report_response =
                                report_task(&self.platform_client, &stream_id, results).await;
    ...
    fn run_judge(&self, task: JudgeTask) -> Result<Vec<JudgeResultInfo>, ClientError> {
        if let Err(sync_err) = self
            .rclone_client
            .sync_bucket("oj-lab-problem-package", PathBuf::from("data"))
        {
            return Err(ClientError::InternalError(sync_err));
        };
        if let Err(set_err) = state::set_busy() {
            return Err(ClientError::InternalError(set_err));
        }
        let problem_slug = task.problem_slug;
        let problem_package_dir = self.package_dir.join(problem_slug.clone());
